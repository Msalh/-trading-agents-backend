...
from app.outcomes import _candidate_anchor_timestamp, _resolve_anchor_timestamp
...
def simulate_barrier_trade(
    ...
    # closing an overdue setup rather than holding it forever.
    last_bar = None
    for bar in forward_bars[:expiry_bars]:
        if bar.get("close") is not None:
            last_bar = bar
    exit_price = last_bar["close"] if last_bar else None
...
def run_barrier_backtest(
    ...
    if summary["gross_loss_usd"] > 0:
        ...
        summary["profit_factor"] = None  # undefined (no losses to divide by) rather than a misleading infinity
...
def compute_backtest_comparison(
    ...
    and returns them side by side — the actual "does Analysis beat
    simple baselines under realistic trade mechanics" comparison the
    external review asked for. Same candidate population and ATR
    geometry held fixed across every source, so any difference in the
    results is attributable to the direction signal alone."""
